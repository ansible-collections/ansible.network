...
class TestVyosPrefixListsModule(TestVyosModule):
    ...
    def test_vyos_prefix_lists_gathered(self):
        ...
        #result = self.execute_module(changed=True)
        # gathered = ','.join(str(result["gathered"]) for v in result["gathered"])
        with open("output.json", "w") as outfile:
            outfile.write(str(result["gathered"]))

        # gathered = ','.join(str(gathered) for v in gathered)
        with open("outfix.json", "w") as outfile:
            outfile.write((str(gathered)))
